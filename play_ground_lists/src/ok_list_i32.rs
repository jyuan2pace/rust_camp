...
impl List {
    ...
    fn helper_append(current: &mut Link, new_n: Link) {
        match current {
            None =>  *current = new_n ,
            Some(n) =>
                List::helper_append(&mut n.next, new_n),
        }
    }
    ...
    pub fn append(&mut self, elem: i32) { 
        ...
        List::helper_append(&mut self.head, new_node);
    }

    fn helper_get_at(current: & Link, idx: i32) -> Option <&i32> {
        match current {
            None => None,
            Some(n) => {
                if idx == 0 {
                    ...
                    List::helper_get_at(& n.next, idx-1)
                }
            },
        }
    }
    pub fn get_at(&self, idx: i32) -> Option<&i32> {
        if idx < 0 {
            ...
            List::helper_get_at(& self.head, idx)
        }
    }

    fn helper_insert_at(current: &mut Link, idx:i32, elem: i32)-> Result<String, String>    {
        if idx == 0 {
            let new_node = Some(Box::new(Node {
                    elem: elem,
                    next: current.take(),
            ...
            *current = new_node;    
            ...
            match current {
                None => Err(format!("out of bounds.")),
                Some(n) => List::helper_insert_at(&mut n.next, idx-1, elem),
            } 
        }
    }
    ...
    pub fn insert_at(&mut self, idx: i32, elem: i32)-> Result<String, String> {
        if idx < 0 {
            Err(format!("out of bounds."))
        } else {
            List::helper_insert_at(&mut self.head, idx, elem)
        }
    }

    fn helper_search(current: & Link, key: i32, idx: i32) -> Option<i32> {
        match current {
            None => None,
            Some(n) => if key == n.elem { Some(idx) } else { List:: helper_search(& n.next, key, idx+1)},
        }

    }
    pub fn search(&self, key: i32) -> Option<i32> {
        List::helper_search(& self.head, key, 0)
    }

    fn helper_delete_at(current: &mut Link, idx: i32) -> Option<i32> {
        match current {
            None => None,
            Some(n) => { 
                if idx == 0 {
                    ...
                    *current=n.next.take();
                    ...
                    List::helper_delete_at(&mut n.next, idx-1)
                }
            },
        }
    }
    pub fn delete_at(&mut self, idx: i32) -> Option<i32> {
        if idx < 0 {
            ...
            List::helper_delete_at(&mut self.head, idx)
        }
    }
